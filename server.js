...
// Manually parse .env as fallback for local execution
...
function serveStaticFile(req, res) {
    ...
    let sanitized = path.normalize(rawUrl).replace(/^(\.\.[/\\])+/, '');

    if (sanitized === '/' || sanitized === '\\' || !sanitized) {
        sanitized = 'index.html';
    }

    let filePath = path.join(PUBLIC_DIR, sanitized);
...
// Master request handler: invoked by Vercel Serverless Function and local HTTP server
...
// Start HTTP server only in local development (Vercel invokes handler directly)
if (!process.env.VERCEL) {
    ...
    server.listen(PORT, () => {
        ...
        console.log(` Key:    ${(process.env.GEMINI_API_KEY || GEMINI_API_KEY) ? 'Configured (hidden)' : 'MISSING'}`);
        console.log(`=================================================`);
    });
}
